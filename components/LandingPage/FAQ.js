...
export default function FAQ() {
    ...
    <div className='flex flex-col justify-center items-center mx-60 mt-40 font-proxima-nova '>
        ...
        <div className="hover:cursor-pointer flex flex-col gap-3 p-5 w-full">
            {faqData.map((faq, index) => (
                <div key={index} className="faq-item border p-3 rounded-lg bg-green_color text-white shadow-md">
                    <div className="flex text-2xl justify-between gap-20 px-10 items-center " onClick={() => toggleAccordion(index)}>
                        <span>{faq.question}</span>
                        {activeIndex === index ? <IoIosArrowDropup /> : <IoIosArrowDropdown />}
                    </div>
                    {activeIndex === index && <div className="text-base mt-3 px-10">{faq.answer}</div>}
                </div>
            ))}
        </div>
    </div>
